...
Version 0.1.x owns one fixed `ao` room and four model-free operations:

- `room.post` — canonically order one message and deduplicate retries by bound principal + stable client message id;
- `room.sync` — return a bounded ascending page after a canonical sequence;
...
- No dynamic rooms, mention wake, attachments, reactions, search, autonomous response, execution, approval engine, Fleet lifecycle, or host routing in this slice.
